...
.token <- function(s='', parent=NULL, prev=NULL, ch='', textmode=TRUE) {
  tok <- new.env()
  tok$s <- s
  tok$args <- list()
  tok$sqarg <- list()
  ...
  while (i < length(str)) {
    ...
    if (ch == '\\') {
      ...
      if (nextch %in% c("[", "]", "{", "}")) {
        ...
        if (nextisarg == 2) {
          nextisarg <- 0
          token <- .token(s='\\', prev=token$parent, parent=token$parent, ch=ch, textmode=textmode)
        } else if (nextisarg == 1) {
          nextisarg <- 2
          ntoken <- .token(parent=token, s=ch, ch=ch, textmode=textmode)
          token$args[[length(token$args)+1]] <- ntoken
          token <- ntoken
        } else {
          token <- .token(s='\\', parent=token$parent, prev=old, ch=ch, textmode=textmode)
        }
      } else {
        ch <- ''
      }
      ...
      if (prevch != ' ') {
        if (nextisarg == 1) {
          nextisarg <- 0
          token <- token$parent
          token <- .token(prev=token, parent=token, ch=ch, textmode=textmode)                    
        } else {
          old <- token
          token <- .token(prev=token, parent=token$parent, ch=ch, textmode=textmode)
          nextisarg <- 0
        }
      }
    } else if (ch == "{" && !prevch=="\\") {
      ...
      if (ch == "'" && prevch == "'")
        next
      if (ch == "'" && nextch == "'")
        token <- .token(s="''", parent=token$parent, prev=token, textmode=textmode)
      else
        token <- .token(s=ch, parent=token$parent, prev=token,textmode=textmode)
      token <- .token(prev=token, parent=token$parent, textmode=textmode)
    } else if (ch == "^" || ch == "_") {
      ...
      if (token$s %in% .supsub) {
        ...
        old <- token
        token <- .token(prev=old, s=ch, parent=old$parent, ch=ch, textmode=textmode)
      }
      ...
      # Any other character
      if (nextisarg == 1) {
        token$args[[length(token$args)+1]] <- .token(s=ch, parent=token, ch=ch, textmode=textmode)
        if (nextch == '^' || nextch == '_') {
