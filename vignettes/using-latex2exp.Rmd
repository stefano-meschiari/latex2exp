...
Since R 4.0, it is recommended to use the new raw string literal syntax (see `?Quotes`). The syntax looks like `r"(...)"`, where `...` can contain any character sequence, including `\`.

Another option is to escape the backslash character (`\`) for LaTeX commands, such that the  command will be written as `\\command` rather than `\command`.
...
You can `plot()` the result from TeX to see a preview of how the expression will be rendered:
```{r euler-identity, eval=FALSE, fig.width=4, fig.height=0.8}
...
```{r ggplot, warning=FALSE, eval=FALSE}
library(purrr)
library(ggplot2)
library(tibble)
